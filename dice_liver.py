...
class check_ji_liver:
    global name_list
    name_list = ['liver_seg.nii']
    ...
    def collect(self,gtPath, predPath):
        ...
        for i in range(len(gtlist)):
            ...
            # open .nii label
            predlist[i] = (predlist[i] + '/liver_seg.nii').replace("\\", "/")
            gtlist[i] = (gtlist[i] + '/liver_seg.nii').replace("\\", "/")
    ...
    def evaluate(self,gt, pred, name, csvpath):
        ...
        df_avg.to_csv(os.path.join('./result',csvpath, 'average_liver.csv'), encoding='utf-8')

        return total_acc / len(gt), total_sen / len(gt), total_spec / len(gt), total_dice / len(gt)

if __name__ == '__main__':
    test = check_ji_liver()
    gtPath = 'D:/desktop/isicdm/dice_ji/dice_liver/gt/'
    predPath = 'D:/desktop/isicdm/dice_ji/dice_liver/pred/'
    stat, gt, pred, name = test.collect(gtPath, predPath)
    if stat == 0:
        print(test.evaluate(gt, pred, name, 'sustc'))
    else:
        print('Error')